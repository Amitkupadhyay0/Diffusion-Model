...
<p align="center">
  <img src="https://github.com/Amitkupadhyay0/Diffusion-Model/blob/main/IMAGES/cat_gen_model.png" alt="Generative Model Example" width="800" height="600">
</p>
...
So , now question is how losse is measured ? 

**Kullback-Leibler (KL) divergence** is a measure that quantifies how one probability distribution differs from another. It is particularly useful in machine learning and statistics to assess how well a predicted distribution approximates the true distribution of data.

1. **Probability Distributions**: KL divergence compares two probability distributions: the true distribution (representing the actual data) and the predicted distribution (generated by a model).

2. **Asymmetry**: KL divergence is asymmetric, meaning that it treats the two distributions differently. This property allows it to provide insights into how one distribution can serve as an approximation for the other.

3. **Interpretation**: KL divergence can be understood as the amount of information lost when using the predicted distribution to represent the true distribution. A lower KL divergence indicates that the predicted distribution closely resembles the true distribution.

4. **Applications**: In the context of diffusion models, KL divergence measures the difference between the model's predictions and the actual data distribution. By minimizing this divergence during training, the model can improve its ability to generate realistic images.
